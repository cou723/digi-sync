...
import { ClassEvent, Inputs } from "../types/types";

const FORM_STATE_DEFAULT_VALUE: Inputs = {
...
export default function ImportForm() {
  let [formState, setFormState] = useState<Inputs>(FORM_STATE_DEFAULT_VALUE);
  ...
  const onImportClick = async () => {
    ...
    if (formState.ignoreOtherEvents) {
      class_events = data.events.filter((e: ClassEvent) =>
        e.className.indexOf("eventJugyo") !== -1
      );
    }
    setImportCount(0);
  ...
  const getEventList = async () => {
    ...
    if (
      res.status_code == "401" && res.detail == "user id or password is invalid"
  ...
  const postToGoogleCalendar = async (class_events: Array<ClassEvent>) => {
    ...
    try {
      ...
      class_events = class_events.filter((class_event) =>
        !isEventDuplicated(already_posted_event_list, class_event)
      );
    } catch (e) {
  ...
  async function getAlreadyPostedEvents(session: any) {
    ...
    do {
      ...
      const google_api_url =
        `https://www.googleapis.com/calendar/v3/calendars/${formState.toCalendar}/events?${
          encodeQueryData(query_param)
        }`;
  ...
  function isEventDuplicated(
    ...
    for (const already_posted_event of already_posted_event_list) {
      ...
      const is_class_title_same =
        class_event.title == already_posted_event.summary;
      const is_start_time_same = new Date(class_event.start).toISOString() ==
  ...
  return (
    <Stack spacing={2} component="form" autoComplete="off" action="/import">
      ...
      <br />
      <Button
        disabled={appState == "unauthenticated" ||
          appState == "connect portal" || appState == "import"}
        variant="contained"
